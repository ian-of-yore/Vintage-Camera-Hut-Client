...
const Products = () => {
    const { data: allProducts = [], } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch('https://rangefinder-server.vercel.app/allProducts');
    ...
    return (
        <div className='w-11/12 mx-auto lg:w-10/12 my-10'>
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {
                    allProducts.length && allProducts.map(product => <Link to={`/trending/${product._id}`} key={product._id}>
                        <div className="card rounded-lg card-compact bg-gray-900 shadow-xl">
                            ...
                            <div className="card-body">
                                ...
                                <div className='flex justify-start items-center'>
                                    ...
                                    <p className='text-left text-base font-semibold'>{product.resellPrice}$</p>
                                </div>
                                <div className='flex justify-start items-center'>
